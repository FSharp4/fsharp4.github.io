...
Both input matrices *pipeline* (visit, and then leave) through the 
...
*simultaneously* and in a "diagonal" fashion, so that the necessary elements 
...
3. These are the correct numbers to produce $$c_{i, j}$$, which is available 
   *immediately after* the required row/column finishes passing through the 
